...
using static HOSPITAL_MANAGEMENT_SOURCE.Report;
    ...
    public partial class FormMainEC : UserControl
    {
        ...
        private void bunifuButtonECPrint_Click(object sender, EventArgs e)
        {
            if (bunifuDataGridViewEC.SelectedRows.Count > 0)
            {
                ...
                //DataTable dt = ExaminationCertificate.GetListEC(Convert.ToInt32(bunifuDataGridViewEC.SelectedRows[0].Cells[0].Value));
                FormReport reportForm = new FormReport();

                //reportForm.ReportData = dt;
        ...
        private void searchExamination()
        {
            try
            {
                //DataView dataView = new DataView(examinationTable);
                ...
                if (!string.IsNullOrEmpty(bunifuTextBoxECSearch.Text))
                {
                    ...
                    ((DataView)bunifuDataGridViewEC.DataSource).RowFilter = "[Mã phiếu khám bệnh] LIKE '*" + bunifuTextBoxECSearch.Text.Trim() + "*' OR "
                                                         ...
                                                         + "[Kết quả] LIKE '*" + bunifuTextBoxECSearch.Text.Trim() + "*' OR "
        ...
        private void bunifuTextBoxECSearch_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void refreshDataViewExamination()
        {
            try
            {
                ...
                examinationTable.Columns.Add("Trạng thái", typeof(string), "IIF([STATE] = 0, 'Chưa xét nghiệm', 'Đã xét nghiệm')");
        ...
        private void bunifuTextBoxECSearch_TextChange(object sender, EventArgs e)
        {
            searchExamination();
        }

        private void bunifuDataGridViewEC_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
