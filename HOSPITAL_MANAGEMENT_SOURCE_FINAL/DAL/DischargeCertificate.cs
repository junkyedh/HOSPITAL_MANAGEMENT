...
namespace HOSPITAL_MANAGEMENT_SOURCE.DAL
{
    public class DischargeCertificate
    {
        ...
        {
            string sqlSelect = @"SELECT        DCID, d.PATIENTID, d.STAFFID, DATE, d.STATE, p.LASTNAME || ' ' || p.FIRSTNAME AS ""PATIENT NAME"", s.LASTNAME || ' ' || s.FIRSTNAME AS ""STAFF NAME""
                                FROM            ""DISCHARGEDCERTIFICATE"" d join ""PATIENT"" p on d.PATIENTID = p.PATIENTID join ""STAFF"" s on s.STAFFID = d.STAFFID";

            //@"SELECT DCID, STAFFID, PATIENTID, DATE, STATE FROM ""DISCHARGEDCERTIFICATE""";
