...
namespace HOSPITAL_MANAGEMENT_SOURCE.GUI
{
    public partial class FormMainUser : Form
    {
        ...
        public FormMainUser(Staff staff)
        {
            ...
            // Bác sĩ 
            if (loginStaff.RoleID == 103)
            {
                bunifuButton2.Enabled = true;
                bunifuButton3.Enabled = true;
            }
            // Quản lý nhân sự 
            if (loginStaff.RoleID == 100)
            {
                bunifuButton1.Enabled = true;
            }
            // Quản lý vật tư/dịch vụ
            if (loginStaff.RoleID == 102)
            {
                bunifuButton4.Enabled = true;   
            }
        }
        ...
        {
            //Console.WriteLine("Nút bệnh nhân đã được click");
            FormMainPatient formMainPatient = new FormMainPatient(loginStaff);
            bunifuShadowPanel1.Controls.Clear();
            bunifuShadowPanel1.Controls.Add(formMainPatient);
            formMainPatient.Dock = DockStyle.Fill;
            formMainPatient.BenhNhan_Click();
        ...
        private void bunifuPictureBox1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    bunifuPictureBox1.ImageLocation = imageLocation; // Sửa lỗi tại đây
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
                MessageBox.Show("Đã xảy ra lỗi ＞﹏＜");
            }
        }
        ...
        {
            FormStaffDetail formStaffDetail = new FormStaffDetail("personalEdit", loginStaff);
            formStaffDetail.ShowDialog();
        }
