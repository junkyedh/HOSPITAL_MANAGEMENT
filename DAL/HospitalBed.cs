...
namespace HOSPITAL_MANAGEMENT_SOURCE.DAL
{
    public class HospitalBed
    {
        ...
        {
            int bedID = GetNextBedID();
            HospitalBed newHB = new HospitalBed();
            String sqlInsert = @"INSERT INTO ""HOSPITALBED""(BEDID,PATIENT,STATE)
                                VALUES        (@BEDID,@PATIENT,@STATE)";

            NpgsqlParameter[] npgsqlParameters =
            {
                new NpgsqlParameter("@BEDID", bedID),
        ...
        public static int GetNextBedID()
        {
            string sqlSelect = @"SELECT MAX(bedid) + 1 AS next_billid FROM ""HOSPITALBED""  ";
